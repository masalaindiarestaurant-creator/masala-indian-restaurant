...
export default function Hero({ locale, copy, slides }: Props) {
  ...
  return (
    <section
      id="home"
      className="relative flex min-h-[max(680px,100svh)] flex-col overflow-hidden bg-dark"
    ...
    >
      ...
      <div className="absolute inset-0 z-10 bg-linear-to-t from-black via-black/45 to-black/30" />
      <div className="absolute inset-0 z-10 bg-linear-to-r from-black/55 via-transparent to-black/30" />
      ...
      <div className="relative z-30 flex min-h-0 w-full flex-1 flex-col">
        ...
        >
          <motion.p
            variants={heroItem}
            className="section-label mb-5 text-(--hero-accent)"
          >
            {copy.eyebrow}
          </motion.p>
          ...
          >
            ...
            />
            <motion.span
              className="text-base text-(--hero-accent)"
          ...
          >
            {copy.titleTop}
            <br />
            <em className="not-italic text-(--hero-accent)">
              {copy.titleAccent}
            </em>
          </motion.h1>
          ...
          >
            <MotionPressable>
              <RouteTransitionLink
                ...
                className="masala-btn masala-btn-filled min-w-48 px-8 py-3.5 text-center text-sm font-semibold text-cream focus:outline-none focus:ring-2 focus:ring-(--hero-accent)/70"
                style={{
                  "--button-fill": "var(--hero-accent)",
                } as CSSProperties}
              >
                {copy.primary}
              </RouteTransitionLink>
            ...
            <MotionPressable>
              <a
                ...
                className="masala-btn min-w-48 bg-black/35 px-8 py-3.5 text-center text-sm font-semibold text-cream backdrop-blur-md focus:outline-none focus:ring-2 focus:ring-(--hero-accent)/70"
                style={{
                  "--button-fill": "var(--hero-accent)",
                } as CSSProperties}
              >
          ...
          {slides.map((_, i) => (
            <motion.button
              ...
              className={`h-1 shrink-0 rounded-full transition-all duration-500 ${
                i === current
                  ? "w-9 bg-(--hero-accent)"
                  : "w-4 bg-cream/35 hover:bg-cream/65"
              }`}
            />
          ))}
        </div>
